...
const dailyRotateFileTransport = createRotateTransport('subscription-%DATE%.log')
const errorFileTransport = createRotateTransport('subscription-error-%DATE%.log', 'error')

// 🔒 创建专门的安全日志记录器
const securityLogger = winston.createLogger({
  level: 'warn',
  format: logFormat,
  transports: [createRotateTransport('subscription-security-%DATE%.log', 'warn')],
  silent: false
})
...
const authDetailLogger = winston.createLogger({
  ...
  transports: [createRotateTransport('subscription-auth-detail-%DATE%.log', 'info')],
  silent: false
})
